...
export function PresenterGuide({ isOpen, onClose }: PresenterGuideProps) {
  ...
  const scenes = [
    ...
    },
    {
      ...
      state: "CRITICAL_HALT (78% Confidence)",
      ...
      script: "Here is the key contradiction! The LLM Critic notices that reporting cooling ON while cargo climbs is physically inconsistent. It flags a contradiction, which costs a fixed two-signal penalty in the confidence formula. The controller drops state to CRITICAL_HALT and requires HUMAN approval before any automated cooling override can execute.",
    ...
    },
    {
      ...
      state: "INSUFFICIENT_DATA (44% Confidence)",
      badgeClass: "badge-warning",
      trigger: () => fleet.killAgent("agent_a", true),
      story: "Agent A (truck telematics) loses network connection or crashes mid-route.",
      script: "If a sensor agent dies or goes offline, the harness does NOT hallucinate or guess. The Trust Gate flags the missing required signal, confidence drops to the share of evidence that survived, and the Controller falls back to INSUFFICIENT_DATA.",
